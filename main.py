...
TOKEN = '6623146885:AAGOI2c68UasKYhIB_Bg0HMexf2X3D0pfzM'
...
def ask_name(update: telegram.update.Update, context):
    if not is_registred(update.message.from_user.id):
        if not update.message.from_user.username:
            update.message.reply_text(
                "У вас нет краткого имени. Сделайте его в настройках телеграма."
            )
            return ConversationHandler.END
...
edit_conv_handler = ConversationHandler(
    ...
    fallbacks=[MessageHandler(Filters.text([BACK_TO_MENU]), entry_menu, pass_user_data=True)]
)
...
def main():
    ...
    dp.add_handler(MessageHandler(Filters.text([MENU, BACK_TO_MENU]), entry_menu))
